...
import registerForPushNotificationsAsync from './registerForPushNotification.js';
...
function HomeScreen() {
  ...
  const getHeartRateFromApple = async() => {
    console.log("Success!");
    const opt = {
      startDate: new Date(Date.now() - 24*60*60*1000).toISOString(), // required ISO8601Timestamp
    ...
    console.log("About to fetch the heart rate");

      await AppleHealthKit.getHeartRateSamples(opt, (err, samples) => {
        if(err){
            console.log(err);
            return;
        }
        console.log(samples);
        setAndAddHeartRate(samples);
    });
  };

  const setAndAddHeartRate = async(data) => {
    let obj = data.reverse();
    if (obj.length === 0) {
      setHeartRate('Not Found');
    } else {
      setHeartRate(obj[0].value);
      //Add it to the database
      let hb = [];
      console.log("Heartbeat : "+obj[0].value);
      ...
      console.log("Hb array is :"+hb);
      if(hb.length>0)
      {
      await setDoc(doc(db, "heart_rate", userId.toString()), {
        heartbeat: hb
    });
console.log("Heart beat information added successfully!");
    }
  ...
  if(heartRate!==0)
  {
    ...
    console.log("Latest heartrate is :"+heartRate);
