...
function HeartRate() {
  ...
  const getHeartRateInfo = async() => {
    ...
    let maxId = await AsyncStorage.getItem("USER_ID");
    querySnapshot.forEach((doc) => {
        console.log(maxId);
        console.log("Doc id :"+doc.id.trim());
        console.log("Info:"+doc.data().heartbeat);
        if(maxId===doc.id.trim())
        {
          console.log("Adding heart rate data");
          setHeartinfo(doc.data());
        }
    });
  };

  console.log("Getting the heart rate info!");
